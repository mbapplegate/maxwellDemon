...
func set_geometry(newRadius:float, lens_height:float, center:Vector2):
	...
	var lensThickness =40*FRONT_THICKNESS + maxX
	
	#Distance from lens at theta=0 to principal plane
	#I want this to be located at Zero
	var apexToPlane = lensThickness/lensIndex
	topShape.position = Vector2(-lensThickness/2.0+apexToPlane,-lens_height/2.0+FRONT_THICKNESS/2.0)
	topShape.shape.size = Vector2(lensThickness-2*FRONT_THICKNESS,FRONT_THICKNESS)
	
	botShape.position = Vector2(-lensThickness/2.0+apexToPlane,lens_height/2.0-FRONT_THICKNESS/2.0)
	...
	flatShape.position = Vector2(-(lensThickness-maxX)-apexToPlane+FRONT_THICKNESS/2.0,0.0)
...
func _ray_hit(photonObj:Object, collPoint:Vector2, collNormal:Vector2, _collider:Object):
	...
	if _collider.name == "FrontFaceArea":
		...
		$Sprite2D2.position = Vector2.ZERO#Vector2(25*cos(correctedTheta),25*sin(correctedTheta)) + locPt
